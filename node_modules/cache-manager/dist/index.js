...
var createCache = (options) => {
  ...
  const mget = async (keys) => {
    ...
    if (nonBlocking) {
      ...
      for (const store of stores) {
        try {
          const missingValues = result.map((value, index) => value === void 0 ? { originalIndex: index, key: keys[index] } : void 0).filter((v) => v !== void 0);
  ...
  const mset = async (stores2, rawList) => {
    const list = rawList.map(({ key, value, ttl: ttl2 }) => ({ key, value, ttl: ttl2 ?? options?.ttl }));
    try {
