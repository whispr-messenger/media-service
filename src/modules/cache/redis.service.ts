...
export class RedisService implements OnModuleInit, OnModuleDestroy {
    ...
    this.client = createClient({
      ...
      socket: {
        reconnectStrategy: (retries) => {
          if (retries > 10) {
            this.logger.error(
              'Trop de tentatives de reconnexion Redis, abandon',
            );
  ...
  async getUserQuota(userId: string): Promise<UserQuota | null> {
    try {
      ...
      this.logger.error(
        `Erreur lors de la récupération du quota pour ${userId}:`,
        error,
      );
      return null;
    }
  ...
  async setUserQuota(
    ...
    try {
      ...
      this.logger.error(
        `Erreur lors de la sauvegarde du quota pour ${quota.userId}:`,
        error,
      );
      return false;
    }
  ...
  async updateUserQuota(
    ...
    try {
      ...
      this.logger.error(
        `Erreur lors de la mise à jour du quota pour ${userId}:`,
        error,
      );
      return false;
    }
  ...
  async deleteUserQuota(userId: string): Promise<boolean> {
    try {
      ...
      this.logger.error(
        `Erreur lors de la suppression du quota pour ${userId}:`,
        error,
      );
      return false;
    }
  ...
  async getMediaMetadata(mediaId: string): Promise<MediaMetadata | null> {
    try {
      ...
      this.logger.error(
        `Erreur lors de la récupération des métadonnées pour ${mediaId}:`,
        error,
      );
      return null;
    }
  ...
  async setMediaMetadata(
    ...
    try {
      ...
      this.logger.error(
        `Erreur lors de la sauvegarde des métadonnées pour ${metadata.id}:`,
        error,
      );
      return false;
    }
  ...
  async deleteMediaMetadata(mediaId: string): Promise<boolean> {
    try {
      ...
      this.logger.error(
        `Erreur lors de la suppression des métadonnées pour ${mediaId}:`,
        error,
      );
      return false;
    }
  ...
  async createDownloadSession(
    ...
    try {
      ...
      this.logger.error(
        'Erreur lors de la création de session de téléchargement:',
        error,
      );
      throw error;
    }
  ...
  async getDownloadSession(
    sessionId: string,
  ): Promise<{ mediaId: string; userId: string; createdAt: Date } | null> {
    try {
      const key = `download:${sessionId}`;
      const data = await this.client.get(key);
      ...
      this.logger.error(
        `Erreur lors de la récupération de session ${sessionId}:`,
        error,
      );
      return null;
    }
  ...
  async deleteDownloadSession(sessionId: string): Promise<boolean> {
    try {
      ...
      this.logger.error(
        `Erreur lors de la suppression de session ${sessionId}:`,
        error,
      );
      return false;
    }
  ...
  async exists(key: string): Promise<boolean> {
    try {
      ...
      this.logger.error(
        `Erreur lors de la vérification d'existence de ${key}:`,
        error,
      );
      return false;
    }
