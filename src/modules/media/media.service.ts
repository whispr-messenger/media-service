...
export class MediaService {
			...
			if (previewData && category.previewEnabled) {
				...
				await this.prisma.mediaPreview.create({
					data: {
						...
						contentType:
							this.getPreviewType(file.mimetype) === 'image' ? 'image/jpeg' : 'video/mp4',
	...
	private async generateImagePreview(filePath: string): Promise<Buffer> {
		return sharp(filePath).resize(300, 300, { fit: 'cover' }).jpeg({ quality: 70 }).toBuffer();
	}
		...
		if (quota) {
			await this.prisma.userQuota.update({
				...
				data: {
					...
					filesCount:
						operation === 'add' ? quota.filesCount + 1 : Math.max(0, quota.filesCount - 1),
				},
			});
		} else {
