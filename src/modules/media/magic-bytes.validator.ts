...
const MAGIC_MAP: Record<string, MagicSignature[]> = {
	...
	// HEIC : brand heic (iPhone ≤13), heix (iPhone 14/15, HEVC extended), hevc (HEVC still image)
	// Tous partagent le même container ISOBMFF - seuls les 4 octets de brand diffèrent.
	'image/heic': [
		[{ bytes: [0x66, 0x74, 0x79, 0x70, 0x68, 0x65, 0x69, 0x63], offset: 4 }], // heic
		[{ bytes: [0x66, 0x74, 0x79, 0x70, 0x68, 0x65, 0x69, 0x78], offset: 4 }], // heix - iPhone 14/15
		[{ bytes: [0x66, 0x74, 0x79, 0x70, 0x68, 0x65, 0x76, 0x63], offset: 4 }], // hevc
	],
	// HEIF : brand mif1 (HEIF still), msf1 (Live Photos / multi-frame sequences)
	'image/heif': [
		[{ bytes: [0x66, 0x74, 0x79, 0x70, 0x6d, 0x69, 0x66, 0x31], offset: 4 }], // mif1
		[{ bytes: [0x66, 0x74, 0x79, 0x70, 0x6d, 0x73, 0x66, 0x31], offset: 4 }], // msf1 - Live Photos
	],
	// Video
	...
	'audio/x-caf': [[{ bytes: [0x63, 0x61, 0x66, 0x66], offset: 0 }]],
	// Documents
